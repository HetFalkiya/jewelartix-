...
def init_db():
    ...
    # Settings table (labor rates, excel)
    cur.execute('''
...
def get_quotations():
    ...
    cur.execute('''
        SELECT id, quot_no, design_no, client_name, quot_date,
               data, (pdf_base64 IS NOT NULL) as has_pdf,
...
def save_quotation():
    ...
    if not quot_no:
        return jsonify({'error': 'quotNo required'}), 400
...
# ── FORM STATE API (auto-save current form) ───────────────────
